...
class TransportTypeUpdateRequest extends FormRequest
{
    ...
    {
        $transportTypeId = $this->editingId ?? $this->route('transport_type') ?? $this->input('transport_type_id');

        return [
            'title_ru' => ['required', 'string', 'max:255'],
            'title_kk' => ['nullable', 'string', 'max:255'],
            'title_en' => ['nullable', 'string', 'max:255'],
            'value' => ['required', 'string', 'max:280', 'unique:transport_types,value,' . $transportTypeId],
            'is_active' => ['boolean'],
        ];
    }
