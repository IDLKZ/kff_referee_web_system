...
<button
    ...
    @click="toggle()"
    class="theme-toggle-btn relative flex items-center justify-center w-9 h-9 rounded-lg transition-all duration-300 hover:scale-105 active:scale-95"
    :title="dark ? '{{ __('ui.light') }}' : '{{ __('ui.dark') }}'"
    style="background: var(--bg-hover); color: var(--text-secondary);"
>
    {{-- Sun icon (shown in dark mode) --}}
    <svg x-show="dark" x-transition:enter="transition-opacity duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition-opacity duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
