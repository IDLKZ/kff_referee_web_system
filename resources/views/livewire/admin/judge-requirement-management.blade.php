@section('page-title', __('ui.judge_requirements'))

<div>
    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 class="text-2xl font-bold" style="color: var(--text-primary);">
            {{ __('ui.judge_requirements') }}
        </h2>
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Create button --}}
            @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::JUDGE_REQUIREMENTS_CREATE))
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th class="cursor-pointer hover:bg-opacity-80 transition-colors" wire:click="sortBy('match')">
                            <div class="flex items-center gap-1">
                                {{ __('crud.match') }}
                                @if($sortField === 'match')
                                    <svg class="w-3 h-3" style="color: var(--color-primary);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <th class="cursor-pointer hover:bg-opacity-80 transition-colors" wire:click="sortBy('judge_type')">
                            <div class="flex items-center gap-1">
                                {{ __('crud.judge_type') }}
                                @if($sortField === 'judge_type')
                                    <svg class="w-3 h-3" style="color: var(--color-primary);" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                              d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}"/>
                                    </svg>
                        ...
                        <th>{{ __('crud.qty') }}</th>
                        <th>{{ __('crud.is_required') }}</th>
                        <th>{{ __('crud.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($items as $item)
                        <tr wire:key="judge-req-{{ $item->id }}">
                            <td style="color: var(--text-muted);">{{ $item->id }}</td>
                            <td>
                                @if($item->match && $item->match->tournament)
                                    <div>
                                ...
                                @else
                                    <span style="color: var(--text-muted);">{{ __('crud.deleted_match') }}</span>
                                @endif
                            </td>
                            <td>
                                @if($item->judgeType)
                                    {{ $item->judgeType->title_ru }}
                                @else
                                    <span style="color: var(--text-muted);">{{ __('crud.deleted_judge_type') }}</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::JUDGE_REQUIREMENTS_UPDATE))
                                    ...
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::JUDGE_REQUIREMENTS_DELETE))
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    ...
                    <button wire:click="sortBy('match')"
                            class="text-left px-3 py-2 rounded-md text-sm transition-colors {{ $sortField === 'match' ? 'btn-primary' : 'btn-secondary' }}">
                        {{ __('crud.match') }}
                        @if($sortField === 'match')
                            <span class="ml-1">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                        @endif
                    </button>
                    <button wire:click="sortBy('judge_type')"
                            class="text-left px-3 py-2 rounded-md text-sm transition-colors {{ $sortField === 'judge_type' ? 'btn-primary' : 'btn-secondary' }}">
                        {{ __('crud.judge_type') }}
                        @if($sortField === 'judge_type')
                            <span class="ml-1">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                        @endif
    ...
    <x-modal wire:model="showFormModal" maxWidth="lg">
        <x-slot name="title">
            {{ $editingId ? __('crud.edit_judge_requirement') : __('crud.create_judge_requirement') }}
        </x-slot>

        <form wire:submit="save">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                {{-- Match searchable select --}}
                <div>
                    <label class="form-label">{{ __('crud.match') }} <span style="color:var(--color-danger);">*</span></label>
                    <div x-data="{ open: false }" @click.away="open = false" class="relative">
                        @if($match_id)
                            <div class="form-input flex items-center justify-between">
                                <span class="truncate" style="color: var(--text-primary);">{{ $selectedMatchName }}</span>
                                <button wire:click="clearMatch" type="button" class="ml-2 shrink-0 p-0.5 rounded transition-colors"
                        ...
                        @else
                            <input type="text"
                                   wire:model.live.debounce.300ms="matchSearch"
                                   @focus="open = true"
                                   placeholder="{{ __('crud.search_match_placeholder') }}"
                            ...
                            <div x-show="open" x-cloak x-transition
                                ...
                                @if(strlen($matchSearch) >= 2)
                                    @forelse($matchOptions as $matchOption)
                                        <button wire:click="selectMatch({{ $matchOption->id }})"
                                                @click="open = false"
                                                type="button"
                ...
                {{-- Judge Type searchable select --}}
                <div>
                    <label class="form-label">{{ __('crud.judge_type') }} <span style="color:var(--color-danger);">*</span></label>
                    <div x-data="{ open: false }" @click.away="open = false" class="relative">
                        @if($judge_type_id)
                            <div class="form-input flex items-center justify-between">
                                <span class="truncate" style="color: var(--text-primary);">{{ $selectedJudgeTypeName }}</span>
                                <button wire:click="clearJudgeType" type="button" class="ml-2 shrink-0 p-0.5 rounded transition-colors"
                                        style="color: var(--text-muted);"
                                        x-on:mouseenter="$el.style.color = 'var(--color-danger)'"
                                        x-on:mouseleave="$el.style.color = 'var(--text-muted)'">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        @else
                            <input type="text"
                                   wire:model.live.debounce.300ms="judgeTypeSearch"
                                   @focus="open = true"
                                   placeholder="{{ __('crud.search_judge_type_placeholder') }}"
                                   class="form-input @error('judge_type_id') is-invalid @enderror">
                            <div x-show="open" x-cloak x-transition
                                 class="absolute z-50 w-full mt-1 max-h-48 overflow-y-auto rounded-lg shadow-lg"
                                 style="background: var(--bg-primary); border: 1px solid var(--border-color);">
                                @if(strlen($judgeTypeSearch) >= 2)
                                    @forelse($judgeTypeOptions as $judgeTypeOption)
                                        <button wire:click="selectJudgeType({{ $judgeTypeOption->id }})"
                                                @click="open = false"
                                                type="button"
                                                class="w-full text-left px-3 py-2 text-sm cursor-pointer transition-colors"
                                                style="color: var(--text-primary);"
                                                x-on:mouseenter="$el.style.backgroundColor = 'var(--bg-hover)'"
                                                x-on:mouseleave="$el.style.backgroundColor = 'transparent'">
                                            <div class="font-medium">{{ $judgeTypeOption->title_ru }}</div>
                                            @if($judgeTypeOption->title_kk || $judgeTypeOption->title_en)
                                                <div class="text-xs" style="color: var(--text-muted);">
                                                    {{ $judgeTypeOption->title_kk }} {{ $judgeTypeOption->title_en ? '/ ' . $judgeTypeOption->title_en : '' }}
                                                </div>
                                            @endif
                                        </button>
                                    @empty
                ...
                {{-- Qty --}}
                <div>
                    <label class="form-label">{{ __('crud.qty') }} <span style="color:var(--color-danger);">*</span></label>
                    <input
                        type="number"
                        wire:model.live="qty"
                        min="1"
                ...
                <div class="flex items-center">
                    <label class="flex items-center gap-2 cursor-pointer mt-6">
                        <input type="checkbox" wire:model.live="is_required" class="w-5 h-5 rounded border-gray-300">
                        <span style="color: var(--text-primary);">{{ __('crud.is_required') }}</span>
                    </label>
                </div>
            </div>
        </form>
