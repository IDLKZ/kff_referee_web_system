@section('page-title', __('ui.category_operations'))

<div>
    {{-- Заголовок --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 class="text-2xl font-bold" style="color: var(--text-primary);">
            {{ __('ui.category_operations') }}
        </h2>
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Кнопка Создать --}}
            @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::CATEGORY_OPERATIONS_CREATE))
            ...
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        {{-- Позиция в цепочке --}}
                        <th>{{ __('crud.chain_position') }}</th>

                        {{-- Предыдущая --}}
                        <th>{{ __('crud.previous_category') }}</th>

                        {{-- Следующая --}}
                        <th>{{ __('crud.next_category') }}</th>
                ...
                <tbody>
                    @forelse($categories as $category)
                        <tr wire:key="cat-{{ $category->id }}">
                            ...
                            {{-- Название --}}
                            <td class="font-medium">{{ $category->title_ru }}</td>
                            ...
                            {{-- Позиция в цепочке (is_first / is_last) --}}
                            <td>
                                <div class="flex items-center gap-1 flex-wrap">
                                    @if($category->is_first)
                                        <span class="badge badge-success text-xs">{{ __('crud.is_first_label') }}</span>
                                    @endif
                                    @if($category->is_last)
                                        <span class="badge badge-danger text-xs">{{ __('crud.is_last_label') }}</span>
                                    @endif
                                    @if(!$category->is_first && !$category->is_last)
                                        <span style="color: var(--text-muted);">{{ __('crud.middle') }}</span>
                                    @endif
                                </div>
                            </td>
                            ...
                            <td>
                                @if($category->previous_id && $category->category_operation)
                                    <span class="text-sm">{{ $category->category_operation->title_ru }}</span>
                                @else
                            ...
                            <td>
                                @php
                                    $nextCat = $category->next_id
                                        ? \App\Models\CategoryOperation::find($category->next_id)
                                        : null;
                                @endphp
                                @if($nextCat)
                                    <span class="text-sm">{{ $nextCat->title_ru }}</span>
                                @else
                                    <span style="color: var(--text-muted);">—</span>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::CATEGORY_OPERATIONS_UPDATE))
                                    ...
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::CATEGORY_OPERATIONS_DELETE))
    ...
    <x-modal wire:model="showFormModal" maxWidth="lg">
        <x-slot name="title">
            {{ $isEditing ? __('crud.edit_category_operation') : __('crud.create_category_operation') }}
        </x-slot>
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <input
                    ...
                    placeholder="{{ __('crud.category_operation_value_placeholder') }}"
                >
                @error('value') <p class="form-error">{{ $message }}</p> @enderror
                <p class="text-xs mt-1" style="color: var(--text-muted);">
                    {{ __('crud.category_operation_value_hint') }}
                </p>
            </div>
            ...
            <div>
                ...
                <p class="text-xs mt-1 ml-6" style="color: var(--text-muted);">
                    {{ __('crud.is_last_hint') }}
                </p>
            </div>

            {{-- Предыдущая категория --}}
            <div>
                <label class="form-label">{{ __('crud.previous_category') }}</label>
            ...
            {{-- Следующая категория --}}
            <div>
                <label class="form-label">{{ __('crud.next_category') }}</label>
