@section('page-title', __('ui.facilities'))
...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Create button --}}
            @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::FACILITIES_CREATE))
                ...
                <tbody>
                    @forelse($facilities as $facility)
                        <tr wire:key="facility-{{ $facility->id }}">
                            <td style="color: var(--text-muted);">{{ $facility->id }}</td>
                            <td class="font-medium">{{ $facility->title_ru }}</td>
                            <td>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::FACILITIES_UPDATE))
                                        <button wire:click="openEditModal({{ $facility->id }})"
                                    ...
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::FACILITIES_DELETE))
    ...
    <x-modal wire:model="showFormModal" maxWidth="md">
        <x-slot name="title">
            {{ $isEditing ? __('crud.edit_facility') : __('crud.create_facility') }}
        </x-slot>
