@section('page-title', __('crud.role_permissions'))

<div>
    {{-- Header --}}
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h2 class="text-2xl font-bold" style="color: var(--text-primary);">
            {{ __('crud.role_permissions') }}
        </h2>
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Create button --}}
            @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::ROLE_PERMISSIONS_CREATE))
    ...
    <div class="card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="data-table">
                <thead>
                    <tr>
                        <th class="cursor-pointer hover:bg-opacity-80 transition-colors" wire:click="sortBy('role_id')">
                        ...
                        <th class="cursor-pointer hover:bg-opacity-80 transition-colors" wire:click="sortBy('permission_id')">
                            <div class="flex items-center gap-1">
                                {{ __('crud.permission') }}
                        ...
                        <th>{{ __('crud.role_code') }}</th>
                        <th>{{ __('crud.permission_code') }}</th>
                        <th>{{ __('crud.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($rolePermissions as $rp)
                        <tr wire:key="rp-{{ $rp->role_id }}-{{ $rp->permission_id }}">
                            <td class="font-medium">{{ $rp->role->title_ru ?? '—' }}</td>
                            <td>{{ $rp->permission->title_ru ?? '—' }}</td>
                            <td>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::ROLE_PERMISSIONS_DELETE))
                                        <button wire:click="confirmDelete({{ $rp->role_id }}, {{ $rp->permission_id }})"
        ...
        @if($rolePermissions->hasPages())
            <div class="px-6 py-4" style="border-top: 1px solid var(--border-color);">
                {{ $rolePermissions->links('vendor.pagination.tailwind') }}
            </div>
        @endif
    </div>
        ...
        <div class="space-y-4">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <button wire:click="sortBy('role_id')"
                            class="text-left px-3 py-2 rounded-md text-sm transition-colors {{ $sortField === 'role_id' ? 'btn-primary' : 'btn-secondary' }}">
                        {{ __('crud.role') }}
                        @if($sortField === 'role_id')
                            <span class="ml-1">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                        @endif
                    </button>
                    <button wire:click="sortBy('permission_id')"
                            class="text-left px-3 py-2 rounded-md text-sm transition-colors {{ $sortField === 'permission_id' ? 'btn-primary' : 'btn-secondary' }}">
                        {{ __('crud.permission') }}
                        @if($sortField === 'permission_id')
                            <span class="ml-1">{{ $sortDirection === 'asc' ? '↑' : '↓' }}</span>
                        @endif
    ...
    <x-modal wire:model="showFormModal" maxWidth="md">
        <x-slot name="title">
            {{ __('crud.create_role_permission') }}
        </x-slot>
