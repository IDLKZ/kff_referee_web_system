@section('page-title', __('ui.judge_types'))
...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Create button --}}
            @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::JUDGE_TYPES_CREATE))
            ...
            <table class="data-table">
                ...
                <tbody>
                    @forelse($items as $item)
                        <tr wire:key="judge-type-{{ $item->id }}">
                            <td style="color: var(--text-muted);">{{ $item->id }}</td>
                            <td class="font-medium">{{ $item->title_ru }}</td>
                            <td>{{ $item->title_kk ?? '—' }}</td>
                            <td>{{ $item->title_en ?? '—' }}</td>
                            <td>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::JUDGE_TYPES_UPDATE))
                                    ...
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::JUDGE_TYPES_DELETE))
    ...
    <x-modal wire:model="showFormModal" maxWidth="lg">
        <x-slot name="title">
            {{ $isEditing ? __('crud.edit_judge_type') : __('crud.create_judge_type') }}
        </x-slot>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model="value"
                           ...
                           {{ $isEditing ? 'disabled' : '' }}
                           placeholder="{{ __('crud.judge_type_value_placeholder') }}">
                    @error('value') <p class="form-error">{{ $message }}</p> @enderror
                    @if(!$isEditing)
                        <p class="text-xs mt-1" style="color: var(--text-muted);">
                            {{ __('crud.judge_type_value_hint') }}
                        </p>
                    @endif
                </div>
