@section('page-title', __('ui.countries'))
...
<div>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div class="flex items-center gap-3">
            ...
            {{-- Create button --}}
            @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::COUNTRIES_CREATE))
                ...
                <tbody>
                    @forelse($countries as $country)
                        <tr wire:key="country-{{ $country->id }}">
                            <td style="color: var(--text-muted);">{{ $country->id }}</td>
                            <td class="font-medium">{{ $country->title_ru }}</td>
                            <td>{{ $country->title_kk ?? '—' }}</td>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::COUNTRIES_UPDATE))
                                    ...
                                    @if(auth()->user()->hasPermission(\App\Constants\PermissionConstants::COUNTRIES_DELETE))
    ...
    <x-modal wire:model="showFormModal" maxWidth="lg">
        <x-slot name="title">
            {{ $isEditing ? __('crud.edit_country') : __('crud.create_country') }}
        </x-slot>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model="value"
                           ...
                           {{ $isEditing ? 'disabled' : '' }}
                           placeholder="{{ __('crud.country_value_placeholder') }}">
                    @error('value') <p class="form-error">{{ $message }}</p> @enderror
                    @if(!$isEditing)
                        <p class="text-xs mt-1" style="color: var(--text-muted);">
                            {{ __('crud.country_value_hint') }}
                        </p>
                    @endif
                </div>
