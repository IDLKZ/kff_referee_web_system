...
@section('page-title', __('ui.my_match_reports'))
...
<div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-xl font-bold" style="color: var(--text-primary);">
                        {{ __('ui.my_match_reports') }}
                    </h1>
                    <p class="text-sm mt-0.5" style="color: var(--text-muted);">
                        {{ __('crud.total') }}: {{ $pendingCount + $myReportsCount }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3">
                ...
                <button wire:click="toggleFilterModal"
                    ...
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-2 2H3.293A1 1 0 003 6.586V4z"/>
                    </svg>
    ...
    <div class="grid grid-cols-2 gap-4 mb-6">
        <button wire:click="setTab('pending')"
                ...
                style="{{ $activeTab === 'pending' ? 'border: 2px solid var(--color-warning); box-shadow: 0 0 0 3px var(--color-warning-light);' : 'border: 2px solid transparent;' }}">
            ...
            <p class="text-xs font-medium mt-1" style="color: var(--text-muted);">{{ __('ui.tab_pending_submission') }}</p>
        </button>

        <button wire:click="setTab('my_reports')"
                class="card p-4 text-center transition-all duration-200 cursor-pointer"
                style="{{ $activeTab === 'my_reports' ? 'border: 2px solid var(--color-success); box-shadow: 0 0 0 3px var(--color-success-light);' : 'border: 2px solid transparent;' }}">
            <div class="w-10 h-10 rounded-full flex items-center justify-center mx-auto mb-2"
            ...
            <p class="text-2xl font-bold" style="color: var(--text-primary);">{{ $myReportsCount }}</p>
            <p class="text-xs font-medium mt-1" style="color: var(--text-muted);">{{ __('ui.tab_my_reports') }}</p>
        </button>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        @forelse($items as $item)
            @if($activeTab === 'pending')
                @include('shared.common.protocol_requirement_referee_card', ['requirement' => $item])
            @else
                @include('shared.common.match_report_referee_card', ['report' => $item])
            @endif
        @empty
            <div class="col-span-full card p-12 text-center">
                ...
                <p class="text-sm" style="color: var(--text-muted);">
                    @if($activeTab === 'pending')
                        {{ __('ui.no_pending_protocols') }}
                    @else
                        {{ __('ui.no_reports_yet') }}
                    @endif
                </p>
            </div>
        @endforelse
